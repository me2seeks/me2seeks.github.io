...
现在`MySql`成为整个流程最慢的一环，`pages`表一天就要增加几百万行，innodb的B=数树高度很快到达三层(高度为3的B+树大概可以存放1170 × 1170 × 16 = 21902400 )。这时就需要对MySql做性能优化。
...
使用Redis 作为词典数据的中转站，在 Redis 中针对每一个词生成一个 List，把倒排出来的索引插入到尾部：
